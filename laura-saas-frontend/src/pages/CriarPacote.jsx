...
function CriarPacote() {
  ...
  useEffect(() => {
    async function fetchCategorias() {
      try {
        const res = await api.get('/pacotes', { params: { limit: 100 } });
        const cats = [...new Set((res.data?.data || []).map((x) => x.categoria).filter(Boolean))].sort(
          (a, b) => a.localeCompare(b, 'pt-PT', { sensitivity: 'base' })
        );
        setCategorias(cats);
      } catch {
        // datalist é apenas conveniência — falhar a buscar categorias não bloqueia o formulário
      }
    }
    fetchCategorias();
  }, []);
    ...
    <div className={`min-h-screen pt-20 pb-8 px-4 ${pageBg}`}>
      <div className="max-w-2xl mx-auto">
        ...
        <div className={`rounded-2xl border shadow-xl p-6 ${isDarkMode ? 'bg-slate-800/50 border-white/10' : 'bg-white border-slate-200'}`}>
          <h1 className={`text-2xl font-bold mb-6 ${textClass}`}>Criar Novo Serviço</h1>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              ...
              <div className="flex justify-between mt-1">
                {errors.descricao ? <p className="text-sm text-red-400">{errors.descricao.message}</p> : <span />}
