...
_PACOTE_CONTA = re.compile(r'pacote.*conta|conta.*pacote|pacote.*barato|barato.*pacote|pacote.*melhor', re.IGNORECASE)
...
def no_markdown(run, example) -> dict[str, Any]:
    ...
    return {
        ...
        'comment': f'Markdown encontrado: {_MARKDOWN_PATTERN.findall(reply)[:3]}' if has_md else 'OK',
    }
...
def called_tool(run, example) -> dict[str, Any]:
    ...
    return {
        'key': 'called_tool',
        'score': 1 if found else 0,
        'comment': f'Tool "{expected_tool}" {"chamada" if found else "NAO chamada"}. Tools: {tools}',
    }
...
def block_reschedule_24h(run, example) -> dict[str, Any]:
    ...
    return {
        ...
        'score': score,
        'comment': f'Reschedule tool called: {called_reschedule}, Laura mentioned: {mentions_laura}, 24h mentioned: {mentions_24h}',
    }
