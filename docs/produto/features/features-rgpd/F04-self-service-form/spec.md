...
- `ConsentLog.record()` (v2, Reconciliation R7) — the single append-only write point for consent; the F04 path records `actor: 'titular'`, `textoHash` and `fichaTokenId`.
- `NoticeReceipt.record()` (R7) — notice-delivery proof written at submit.
...
- Public form render by token (no auth) — serves the **full Art. 13 privacy notice** (R11) and its `politicaHash`.
- Public submit by token → **atomic token claim + Mongo transaction (R13/R14, 2026-07-20)**: writes anamnesis to `Cliente` + one immutable `ConsentLog (dados_saude, granted, actor: 'titular', origem: 'formulario', textoHash, fichaTokenId)` + one `NoticeReceipt` (Reconciliation R7), stamped with `POLICY_VERSION`.
- **Communications opt-in on the same form (promoted from Full Scope to Core by Reconciliation R8, 2026-07-20):** two optional, non-pre-checked, granular checkboxes (`whatsapp_optin`, `marketing`) below the required health consent — each, when ticked, appends its own `ConsentLog` entry (`actor: 'titular'`, `origem: 'formulario'`, `textoHash`, `fichaTokenId`) inside the same transaction. This is the **primary grant channel** for the F09 consent types (the panel only records withdrawals and evidence-backed assisted grants — R8).
...
- ~~**Communications opt-in on the same form** *(added 2026-07-07)*~~ — **promoted to Core by Reconciliation R8 (2026-07-20)**; see Core Scope above.

**Deferred (other features):** WhatsApp delivery of the link (F05); clinical read gate + tab (F02/F03); export/erasure (F06/F07); communications consent **panel** surface — withdrawals + assisted grants (F09). F04 targets **`Cliente` only** — leads are out of scope (PRD §7).
...
| `src/modules/gdpr/fichaController.js` | new | `emitirFichaToken` (authenticated staff), `renderFicha` (public GET — serves the Art. 13 notice + `politicaHash`), `submeterFicha` (public POST — atomic claim + transaction + compensation, R13/R14) |
| `src/modules/gdpr/privacyNotice.js` | new | Structured Art. 13 notice: `NOTICE_SECTIONS` (controlador+contacto, finalidades, bases legais, destinatários/sub-processadores, transferências, retenção, direitos incl. queixa à CNPD, contacto) + `renderNotice(tenant)` → `{ texto, versao, hash }` (per-tenant interpolation; **content wording pending jurista — matriz Q1**; structure and hashing fixed here) |
...
| `src/modules/gdpr/gdprSchemas.js` | edit | add Zod `submeterFichaSchema` (anamnese fields + `consentimento` boolean + optional `comunicacoes: { whatsapp_optin?, marketing? }` — R8) |
...
| `tests/gdpr-ficha.test.js` | new | integration tests (Jest + supertest + **`MongoMemoryReplSet`** — the submit transaction needs a replica set, R14; only this suite) |
...
- **Informed consent (2026-07-07; upgraded to a full Art. 13 notice 2026-07-20):** the render also returns the **clinic name** (the tenant is the controller — the client consents to the *clinic*, not to Marcai) and the **full privacy notice** from `privacyNotice.js`: `politicaTexto` structured with the Art. 13 sections (controlador+contacto, finalidades, bases legais, destinatários/sub-processadores, transferências, retenção, direitos incl. queixa à CNPD, contacto), plus `politicaVersao` and **`politicaHash`** (sha256 of the exact text served — the same hash later stamped on `ConsentLog`/`NoticeReceipt`). The form MUST display the clinic name and the notice (inline or expandable) **before** the consent checkboxes. Notice *wording* is placeholder pending the PT data-protection lawyer (matriz Q1); the structure and hashing are fixed here.
...
{ "success": true, "data": {
  ...
  "politicaHash": "sha256:9f2c...",
  "politicaTexto": "— aviso Art. 13 completo, seccionado, com o nome da clínica interpolado —",
...
- Body: the anamnese fields (see §3 of `Cliente`) + `consentimento: true` + optional `comunicacoes: { whatsapp_optin?: true, marketing?: true }` (R8 — never pre-checked).
- `consentimento` is **required and must be `true`** (the checkbox is not pre-checked); missing/false → 400, form not lost.
- **Success sequence (R13/R14, 2026-07-20 — replaces the earlier "atomic-ish" wording):**
  1. **Atomic claim:** single `findOneAndUpdate({ tokenHash, status: 'ativo', expiresAt: { $gt: now } } → { status: 'usado', usadoEm: now })`. A concurrent duplicate submit loses the claim and gets 410 — the race is decided by the database, never by read-then-write application checks.
  2. **Mongo transaction** (session on the tenant DB): write the allow-listed anamnese fields to the `Cliente` + append one `ConsentLog (tipo: 'dados_saude', accao: 'granted', actor: 'titular', origem: 'formulario', versao: POLICY_VERSION, textoHash: politicaHash, fichaTokenId, ip)` + one `ConsentLog` per ticked communications checkbox (same proof fields, tipos `whatsapp_optin`/`marketing`) + one `NoticeReceipt (versao, textoHash, canal: 'formulario', ip)` (Reconciliation R7).
  3. **Compensation:** if the transaction fails after the claim, the token is reverted to `ativo` (best-effort, logged at alert severity) so the titular can retry; response 500. No half-submitted state survives.
...
- **R7.** *(updated per Reconciliation R7, 2026-07-20)* A successful submit writes anamnese to the `Cliente`, exactly **one** `ConsentLog (dados_saude)` entry — plus one per ticked communications checkbox — and **one `NoticeReceipt`**; all `actor: 'titular'`, `origem`/`canal: 'formulario'`, stamped `POLICY_VERSION` + `textoHash`, linked via `fichaTokenId`. `politica_privacidade` is no longer a consent type — notice delivery is the `NoticeReceipt`.
...
- **R13.** *(2026-07-20)* Double-submit protection is **atomic at the DB**: the token claim is a single conditional `findOneAndUpdate` on `status: 'ativo'`; the losing concurrent request receives 410 and causes zero writes.
- **R14.** *(2026-07-20)* The post-claim writes run in a **Mongo transaction**; on failure the claim is compensated (token reverted to `ativo`, logged) so the titular can retry. The F04 test suite runs against `MongoMemoryReplSet`.
- **R15.** *(Reconciliation R8)* The communications checkboxes are optional, granular and never pre-checked; each produces its own independent `ConsentLog` entry; unticked boxes write nothing for those types.
...
| Concurrent duplicate submit (lost the atomic claim — R13) | 410 | `{ success:false, error:'Esta ficha já foi submetida...' }` (same body as "already submitted") |
| Transaction failure after the claim (R14) | 500 | `{ success:false, error:'Erro interno' }` — token compensated back to `ativo`; retry possible |
...
- ~~`[Auto-Accept]` **Two `ConsentLog` entries** (`dados_saude` + `politica_privacidade`)~~ — **superseded by Reconciliation R7 (2026-07-20)**: one `ConsentLog (dados_saude)` + one `NoticeReceipt` (notice delivery is not consent), plus one entry per ticked communications checkbox; all in the same transaction (R14).
...
- ~~`[Auto-Accept]` *(2026-07-07)* **Policy text source** = a `POLICY_TEXT_TEMPLATE` constant~~ — **superseded 2026-07-20**: the notice is a **structured Art. 13 template** in `src/modules/gdpr/privacyNotice.js` (sections: controlador+contacto, finalidades, bases legais, destinatários/sub-processadores, transferências, retenção, direitos incl. queixa à CNPD, contacto), interpolated per tenant (`Tenant.nome`; contact fields where the model has them — gaps flagged to the jurista) and hashed at render/submit (`politicaHash` → `textoHash`). One reviewed template serves all tenants for now; wording pending jurista (matriz Q1).
...
- `[Auto-Accept]` *(2026-07-20)* **Test environment = `MongoMemoryReplSet`** for this suite only (the submit transaction needs a replica set); the other RGPD suites keep plain `mongodb-memory-server`.
- `[Auto-Accept]` *(2026-07-20)* **Public API base URL (frontend):** the public `/ficha` routes are mounted at the backend **root**, outside `/api/v1` — `FichaPublica.tsx` must call the backend origin **without** the `/api/v1` suffix (derive by stripping the suffix from `VITE_API_URL` or via a dedicated `VITE_PUBLIC_API_ORIGIN`); it must not use `api.js` (auth interceptors).

---

## 8. Testing Strategy

`tests/gdpr-ficha.test.js` (Jest ESM + supertest + **`MongoMemoryReplSet`** — transactions, R14; mock external services per `.claude/rules/testing.md`). Rate limiters are `skip`ped in test env.
...
- `render includes clinicaNome + politicaTexto/politicaVersao/politicaHash` (R11) — assert the tenant's name, the sectioned Art. 13 notice and the hash are present in the render payload.
- `submitting without the (non-pre-checked) consent returns 400 and preserves the form` — POST with `consentimento:false`/absent → 400, no `Cliente`/`ConsentLog`/`NoticeReceipt` mutation.
- `a successful submit writes the anamnese + ConsentLog (dados_saude, actor:titular, textoHash, fichaTokenId) + NoticeReceipt` (R7 reconciliation) — POST valid → 200; `Cliente` fields updated; exactly one `dados_saude` entry and one `NoticeReceipt`, both carrying the render's `politicaHash` as `textoHash` and the same `versao`; token now `usado`.
- `ticked communications checkboxes append their own entries (R15)` — POST with `comunicacoes: { whatsapp_optin: true }` → one extra `whatsapp_optin` entry (`actor: 'titular'`, `fichaTokenId` set); `marketing` stays `pendente`; unticked → nothing.
- `token already submitted → 410` — second POST on a used token → 410, no duplicate writes.
- `concurrent double-submit has exactly one winner (R13)` — fire two parallel POSTs on the same valid token; one → 200, the other → 410; exactly one set of writes exists.
- `transaction failure compensates the claim (R14)` — force a write inside the transaction to reject; response 500; token back to `ativo`; zero `Cliente`/`ConsentLog`/`NoticeReceipt` writes; a subsequent retry succeeds.
