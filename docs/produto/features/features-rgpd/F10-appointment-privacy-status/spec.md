...
**Complexity:** moderate
**Module:** frontend-first — `laura-saas-frontend/src/` (agenda, appointment detail, client record, lead-closing) + a lightweight read addition to `src/modules/gdpr/` + **one new `Cliente` field (`notaOperacional` — R10)**
...
> **R10 (2026-07-20) — authoritative over any conflicting text below:** the note surfaced on appointments is the **new `Cliente.notaOperacional`** (String, max 200) — **not** `Cliente.observacoes`. `observacoes` is free text visible to all staff **and injected into the AI prompt** (`ia-service/services/client_orchestrator.py`); promoting it on the agenda invited health data into a field that flows to OpenAI/Google. `notaOperacional` carries the UI guidance *"Nota operacional — sem dados de saúde"*, **never reaches `ia-service`** (internal projections + direct reads, regression-asserted — same invariant class as R1) and is never returned by `/clinico`. `observacoes` is untouched by F10 (legacy; its own AI-flow decision = matriz Q6).
...
- **Existing data:** `GET /agendamentos` (already populates `cliente` — its `select` gains `notaOperacional`); `PUT /clientes/:id` (update path — gains `notaOperacional` in the allow-list).
...
- The client's short **note** (`Cliente.notaOperacional` — R10, new field, max 200) shown alongside the badge on the appointment, labelled in every write surface with *"Nota operacional — sem dados de saúde"* (placeholder examples: preferências de horário, contexto comercial).
- At the **lead-closing / treatment-scheduling** moment and on the **client record**: the `notaOperacional` input (persisted via the existing cliente update path, field added to its allow-list) and a **"Enviar ficha de consentimento"** action that invokes F05.
- **AI-exclusion invariant (R10):** `notaOperacional` is added to no internal/AI projection — `/api/internal/clientes/*` and the direct `db.clientes` read in `client_orchestrator.py` must never include it; regression-asserted in both suites.
...
| `src/models/Cliente.js` | edit | **R10:** add `notaOperacional: { type: String, trim: true, maxlength: 200, default: '' }` |
| `src/modules/clientes/` (controller/schemas + agendamento populate) | edit | **R10:** add `notaOperacional` to the cliente-update allow-list and to the `GET /agendamentos` cliente populate `select`; add it to **no** internal/AI projection |
| `laura-saas-frontend/src/pages/Agendamentos.jsx` | edit | After loading appointments, collect unique `cliente._id`s, fetch the **batch** status once, render `<PrivacyStatusBadge>` + the client note (`ag.cliente.notaOperacional`) on each card. Only for appointments that have a `cliente` |
| `laura-saas-frontend/src/components/AppointmentDetailModal.jsx` | edit | Show `<PrivacyStatusBadge>` + the `notaOperacional` in the detail overlay (client appointments only); add a quick "Enviar ficha de consentimento" affordance that calls F05 |
| `laura-saas-frontend/src/pages/EditarCliente.jsx` | edit | In the "Dados do Cliente" tab: show `<PrivacyStatusBadge>` + a **`notaOperacional`** input (labelled "Nota operacional — sem dados de saúde", max 200) saved via `api.put('/clientes/:id')`; keep the existing F05 **"Enviar ficha…"** button (reused, not duplicated). The legacy `observacoes` textarea stays as-is |
| `laura-saas-frontend/src/components/FunilAvaliacaoModal.jsx` | edit | At lead-closing (status `Compareceu` + lead → `POST /agendamentos/:id/fechar-pacote`): add the **`notaOperacional`** input (same label/guidance) persisted to the **converted client**, and a **"Enviar ficha de consentimento"** button (F05) enabled once the converted client id exists |
| `ia-service` tests + `tests/` | edit | **R10 regression:** assert `notaOperacional` never appears in `/api/internal/clientes/*` payloads nor in the `client_orchestrator.py` projection |
...
Pattern references: F01 (`src/modules/gdpr/*`, `ConsentLog`, latest-by-type index `{ tenantId:1, tipo:1, createdAt:-1 }`), F04 (`src/models/FichaToken.js` status lifecycle), F05 (`POST /gdpr/clientes/:id/enviar-ficha`, the EditarCliente button), F09 (`GET /gdpr/clientes/:id/consent-estado` — the derivation idiom this mirrors), `laura-saas-frontend/src/pages/Agendamentos.jsx` (card + inline status-pill idiom), `src/components/AppointmentDetailModal.jsx`, `EditarCliente.jsx` (tabs idiom), `services/api.js`, `contexts/AuthContext` (`useAuth().user.role`), `.claude/rules/react-components.md` (design system, `.tsx` for new, luxon, no `alert()`).

---

## 3. Data Model

**One schema change (R10):** `Cliente.notaOperacional` (String, trim, **max 200**, default `''`) — the short operational note. Existing docs need no backfill (default empty). Otherwise F10 reuses, read-only:
...
- ~~`Cliente.observacoes` as the note~~ — **superseded by R10**: `observacoes` is untouched by F10 (legacy field that feeds the AI's durable team notes; its fate = matriz Q6). The clinical free-text lives in the separate `observacoesAdicionaisAnamnese`, which F10 never touches. `notaOperacional` never enters AI projections nor `/clinico`.
...
{ "success": true, "data": {
  ...
  "notaOperacional": "Cliente prefere manhãs; fechou pacote 10 sessões."
} }
```
- `estado ∈ { "granted" | "withdrawn" | "pendente" }`; `data` is `null` when `pendente`.
- **Never** returns anamnese/clinical fields. `notaOperacional` is the non-clinical note (included here for the detail modal / record convenience — R10).
...
```
- The batch omits `notaOperacional` — the agenda already has `ag.cliente.notaOperacional` from the populated `GET /agendamentos` response (select updated by F10; no duplication).

### Reused (no new endpoint)
- `PUT /clientes/:id` (existing) — persists the `notaOperacional` note (field added to the update allow-list by F10; record + lead-closing).
...
- **R4.** *(rewritten by R10, 2026-07-20)* The note is the **new `Cliente.notaOperacional`** (max 200), persisted through the existing `PUT /clientes/:id` path (field added to its allow-list). Every write surface carries the guidance **"Nota operacional — sem dados de saúde"**. `notaOperacional` is excluded from every internal/AI projection (regression-asserted) and from `/clinico`; `Cliente.observacoes` is untouched by F10.
- **R5.** At lead-closing, the observation is written to the **converted** client's `Cliente.notaOperacional`, and "Enviar ficha de consentimento" targets that converted client id (the action is enabled only once a client record exists).
...
- ~~`[Auto-Accept]` **The note reuses `Cliente.observacoes`** (no new field).~~ — **superseded by Reconciliation R10 (2026-07-20)**: the note is the new `Cliente.notaOperacional` (max 200, guidance label, excluded from AI projections); `observacoes` is untouched by F10. The clinical free-text `observacoesAdicionaisAnamnese` is never surfaced here.
- `[Auto-Accept]` **Lead-closing host = `FunilAvaliacaoModal.jsx`** (the existing lead-conversion surface tied to `POST /agendamentos/:id/fechar-pacote`). The observation persists to the converted client's `notaOperacional` via `PUT /clientes/:id`; the send button targets the converted client id. If the conversion flow lives in a different component at build time, the same two affordances attach there instead.
...
- `[Auto-Accept]` **Batch omits `notaOperacional`** (already present in the populated agenda response); the single read includes it for the modal/record.
...
- `appointment status shows form (✓ preenchida / ⏳ pendente) + health-consent + note` — seed a client with a `usado` `FichaToken` and a `dados_saude` `granted` `ConsentLog` → single read returns `fichaPreenchida:true`, `consentimentoSaude.estado:'granted'`, `notaOperacional` present; seed a client with only an `ativo` token → `fichaPreenchida:false`, `fichaEnviada:true`, `consentimentoSaude.estado:'pendente'`.
...
- `note persists to Cliente.notaOperacional` — `PUT /clientes/:id` with `notaOperacional` → single read reflects the saved note (>200 chars → 400); `notaOperacional` absent from `/api/internal/clientes/*` payloads (R10 regression).
