...
- Error → raw `{ "message": "…" }`; surfaced as a toast (the `api.js` axios interceptor, which fires automatically for all non-`/auth/` URLs — `scheduleService.ts` re-throws) and an inline error state.
...
- **R5.** Empty/closed days show a clear empty state ("Sem horários disponíveis para esta data") rather than an error; a backend error shows an inline error + toast (from the `api.js` axios interceptor, which fires automatically for all non-`/auth/` URLs; `scheduleService.ts` re-throws) and does not crash the form.
...
| `getAvailableSlots` request error (`{ message }` / network) | Inline error under the picker + toast from the `api.js` axios interceptor (`scheduleService.ts` re-throws; the interceptor fires for all non-`/auth/` URLs); the form stays usable; submit blocked until a valid slot/date is chosen |
...
- **[Auto-Accept] D9 — `SlotPicker.tsx` must follow the project's TypeScript guidance.** Per `CLAUDE.md` rule §6 ("TypeScript: tipos de domínio expressivos") and the `typescript-advanced-types` skill (`.claude/skills/typescript-advanced-types`): (a) type props and state with explicit union/literal types — e.g. `type SlotEstado = 'livre' | 'ocupado' | 'pausa' | 'fora'`, `Record<string, SlotEstado>` for the candidate-classification map; (b) prefer `unknown` over `any`; (c) avoid `as` casts — use narrowing or type guards instead; (d) keep types proportionate to the component's simplicity — do not over-engineer.
