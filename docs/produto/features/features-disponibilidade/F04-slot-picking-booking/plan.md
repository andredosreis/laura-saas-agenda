...
2. **Add `getDiaDisponibilidade(date, duration)`** to `scheduleService.ts`: in parallel call `getAvailableSlots(date, duration)` and `getSchedules()`; from `getSchedules` derive the day's base window/pause (`disponibilidade[weekday]`) and the day's bookings (`agendamentos` filtered to `date`, `Europe/Lisbon`). Return `{ slots: string[], janela: { startTime, endTime, breakStartTime, breakEndTime } | null, ocupados: {start,end}[] }`. When `getSchedules` lacks the date, return `janela: null`/`ocupados: []` (best-effort fallback). Export a `SlotEstado = 'livre'|'ocupado'|'pausa'|'fora'` type. (`spec §4`, `[Auto-Accept] D3`.)
...
4. **`CriarAgendamento.jsx`** (Sessão + Avaliação forms):
   ...
   - Render `<SlotPicker date={dataSelecionada} duration={60} value={…} onChange={…} allowForce={isAdmin} />` (admin from `useAuth()`); on slot pick, compose `dataHora` = `${dataSelecionada}T${slot}` and `setValue('dataHora', …, { shouldValidate: true })` so the existing Zod future-date rule still runs.
...
5. **`QuickAppointmentModal.jsx`**:
   - Replace the `datetime-local` `dataHora` field with a date row (seeded from `selectedDate` via the existing luxon effect) + `<SlotPicker>`; on pick, set `formData.dataHora` in the same `"yyyy-MM-dd'T'HH:mm"` format so `onSubmit(submitData)` is unchanged.
