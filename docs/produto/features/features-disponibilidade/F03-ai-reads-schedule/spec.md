...
| `ia-service/src/ia_service/services/marcai_client.py` | edit | Add a **synchronous** `fetch_available_slots(tenant_id, *, date=None, from_=None, to=None, days=7, duration=60)` using `httpx.Client` + `x-service-token` header (the agent tool is sync). Returns the parsed `data` payload on success; **raises on HTTP/transport error** (the caller, `find_available_slots`, is the error boundary). |
| `ia-service/src/ia_service/services/mongo_reader.py` | edit | Rewire `find_available_slots(...)` to call `marcai_client.fetch_available_slots` and **reshape** the response into the existing `list[{date,time,weekday,iso}]` contract; **catches** transport/HTTP error (raised by `fetch_available_slots`) and returns `[]` on error or `scheduleConfigured: false` (graceful — `find_available_slots` is the error boundary; do not collapse the `[]` fallback into `fetch_available_slots`). Stop importing `agent_business_rules`. |
...
1. Call `marcai_client.fetch_available_slots(tenant_id, days=dias_a_frente, duration=slot_duration_min)` (base URL from `settings.marcai_api_url`, header `x-service-token`). **`fetch_available_slots` raises an exception on any HTTP/transport error — it does not swallow errors.**
...
3. **`find_available_slots` is the error boundary:** it wraps the call in try/except, catches any error raised by `fetch_available_slots`, and returns `[]` on HTTP error, timeout, or `scheduleConfigured: false` (graceful degradation — two distinct layers; do not collapse the `[]` fallback into `fetch_available_slots` itself). Signature & return shape are unchanged, so `lead_tools.get_available_slots` / `client_tools` need no edits.
...
- **R3. Parity with `getAvailableSlots`.** For a given `(tenantId, date, duration)`, the endpoint's `slots` array equals what the legacy `GET /schedules/available-slots` returns for that date — both call the same `resolveAvailableSlots` helper (Node whitelist: `status ∈ ['Agendado','Confirmado']` + `confirmacao.tipo ≠ 'rejeitado'`). This is **parity with the backend `getAvailableSlots`**, not byte-parity with the old Python `find_available_slots` (which used a blacklist `status ∉ [cancelados...]`). The behavioral difference (acknowledged in D12) is that statuses like `Realizado`/`Não compareceu` no longer block slots — practical impact is limited to non-future dates.
...
- **R8. Migration safety.** The migration is **dry-run by default** (prints intended writes, labelling each base `Schedule` day as `"would write"` or `"preserved (customizado)"`), applies only with `--apply`, is **idempotent** (re-running changes nothing — upsert keyed by `(tenantId, dayOfWeek)` and `(tenantId, data)`), and **reversible** (`--rollback` removes the seeded exceptions and restores untouched-day defaults). It must run **before** the AI is flipped over so behavior does not regress. The migration **preserves owner-customized base `Schedule` days**: it only writes a weekday doc if it is still at the `initializeSchedules` default (untouched since F01); customized days are skipped and logged as `"preserved (customizado)"`. A `--force` flag (distinct from `--apply`) overrides the preservation check and writes all days. `ScheduleException` seeding is unaffected (always new data — no overwrite risk).
...
- **[Auto-Accept] D8 — Migration file `src/migrations/seedScheduleFromAgentRules.js`; dry-run default, `--apply`, `--rollback`, `--force`; idempotent upserts with customization guard.** Matches the existing `src/migrations/` script style (`dotenv-flow`, direct model use). Idempotency keyed by `(tenantId, dayOfWeek)` for base days and the unique `(tenantId, data)` for exceptions; `--rollback` removes seeded exceptions and resets seeded days to defaults. The migration skips any base `Schedule` day that differs from the `initializeSchedules` default (owner-customized); dry-run labels each day `"would write"` or `"preserved (customizado)"`. `--force` (distinct from `--apply`) overrides the preservation check. `ScheduleException` seeding is unaffected.
...
- **[Auto-Accept] D12 — Status-filter unified on the Node `getAvailableSlots` whitelist; Python blacklist retired.** The Python `find_available_slots` historically blocked slots via a blacklist (`status ∉ ['cancelado'...]`); the Node `getAvailableSlots` uses a whitelist (`status ∈ ['Agendado','Confirmado']` + `confirmacao.tipo ≠ 'rejeitado'`). F03 unifies on the Node whitelist as the single source of truth; the Python blacklist is retired. **Behavioral difference:** statuses like `Realizado` and `Não compareceu` (post-hoc/past) no longer block future slots. Practical impact is limited to non-future dates (those statuses apply to past appointments), so the AI's forward-looking suggestions are unaffected. This is an **accepted, documented behavior change** — not a silent regression. R3's "parity" means parity with `getAvailableSlots`, not with the old Python behavior.
- **[Auto-Accept] D13 — `Schedule` composite index prerequisite.** The `Schedule` model comment promises a `{ tenantId, dayOfWeek }` unique index ("único por tenant (ver índice composto abaixo)") but the index is currently missing from `src/models/Schedule.js`. F03's migration relies on `(tenantId, dayOfWeek)` uniqueness for idempotent upserts; adding this index is a prerequisite code fix included in Phase 1.
